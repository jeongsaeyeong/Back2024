<?php
    ...
    if ($selectedOption = "" || $selectedOption = "제목만"){
        $titleSql = "SELECT * FROM drinkboard WHERE boardTitle LIKE '%$side_search%' AND boardCategory='커뮤니티'";
        $titleInfo = $connect -> query($titleSql);
    } else {
        $AuthorSql = "SELECT * FROM drinkboard WHERE boardAuthor LIKE '%$board_search%' AND boardCategory='커뮤니티'";
...
<?php foreach($titleInfo as $title){ ?>
    <li>
        <a href="board_view.php?boardId=<?=$title['boardId']?>" target="_blank">
